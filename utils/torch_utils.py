...
"""
当有 GPU(torch.cuda.is_available() 为真)时，通过 torch.cuda.synchronize() 来确保 GPU 上的操作完成，
从而获得更准确的时间点。然后返回当前的系统时间（通过 time.time() 获取）。
这样可以在涉及到 GPU 计算的情况下，获得更可靠的时间测量，避免因为 GPU 计算和 CPU 时间记录不同步而导致的不准确。
